...
-->
    ...
    <xsl:template match="author">
        ...
        <!-- Clean up the string -->
        <xsl:variable name="authorString" select="."></xsl:variable>
        <xsl:variable name="len" select="string-length($authorString)"></xsl:variable>
        <xsl:variable name="lastChar" select="substring($authorString,$len -2)"/>
        <xsl:variable name="rawString">
           <xsl:choose>
                <xsl:when test="contains($lastChar, ',')">
                    <xsl:value-of select="substring($authorString,1, $len - 1)"/>
                </xsl:when>
                <xsl:otherwise>
        ...
        <xsl:variable name="authors-trimmed">
            <xsl:choose>
                <xsl:when test="contains($authorsIn,'20')">
                    ...
                    <xsl:value-of select="substring($authorsIn,1,$l - 1)"/>
                </xsl:when>
                <xsl:otherwise>
