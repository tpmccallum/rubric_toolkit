...
Syntax
python 
archive_to_foxml_vital_2.py archiveName startNum PIDPrefix
...
Example
python 
archive_to_foxml_vital_2.py dsapceArchive 0 vital exportedItems Imported_Items A marc.xml A dublin_core.xml A False False http://servername/directoryname
"""
...
def  main(archiveName, num, pidPrefix, outputDirectory, labelPrefix, foxmlState, marcFile, marcState, dcFile, dcState, modsFile, modsState, baseUrl):
    ...
    for item in arc.items:
        ...
        foxy.addInlineXML("DS1", "MARCXML Record", marc, marcState)
        #add datastream mods
        if modsFile != "False" or modsState != "False":
            mods = libxml2.parseDoc(item.readFile(modsFile))
            foxy.addInlineXML("DS2", "MODSXML Record", mods, modsState)
